...
| **H2b** | **Codec trait extraction. BUILT 2026-09-15** (intent `INT-2026-09-14-001`). Extracted the codec trait pair (request-mask walk, response demask, SSE demask, route classification, per-codec header policy); items 3-6 of §1.2 became the Anthropic impl under `crates/vg-proxy/src/codec/anthropic/`; `FORWARDED_HEADERS` moved into the Anthropic codec under F4's resolved policy (prefix allowlist + named singletons + credential-shaped denylist that wins over any prefix match — see `docs/decisions.md`'s 2026-09-15 entry for the full record, including a real defense-in-depth trade-off in `upstream::forward` found by cross-model review and documented rather than reversed). | 1-2 | — (starts immediately, parallel to H1) | **Met.** Zero Anthropic-shaped types outside the Anthropic codec module (grep-verified); all existing tests green; `scripts/a2-live-proof.sh` unregressed. |
| **H2c** | **Bounds and timeouts. BUILT 2026-09-15** (intent `INT-2026-09-14-001`, built on H2b's own branch since both touch `upstream.rs`/`server.rs`). Closed §1.2 item 9 in **both** directions: a request-body bound (`server.rs::collect_body`/`MAX_REQUEST_BODY_BYTES`, 32 MiB) and a response-body bound (`upstream.rs::MAX_RESPONSE_BODY_BYTES`, 64 MiB), plus **five separately named timeouts** (`upstream.rs::Timeouts`): connect, response-headers, idle-body, total-request, and streaming-idle (300s, matching GROUND-13/14's real vendor numbers). Two review rounds (single-model + Codex cross-model) found and fixed 6 real issues — see `docs/decisions.md`'s 2026-09-15 entry. | 1-2 | — (starts immediately) | **Met.** A long-running real stream survives (proven by `scripts/a2-live-proof.sh` and a dedicated fast unit test proving the idle-reset mechanism); an oversized body in either direction fails closed with a 4xx/5xx (413/502), not an OOM; each of the five timeouts has its own dedicated, isolated test in `tests/upstream_timeouts.rs` (8 tests total). |
| **H3** | **OpenAI Responses codec.** Route table, request-tree mask walk, response + SSE-event demask for the Responses schema, built against H1's real transport/path/header-name findings and its hand-authored synthetic structural corpus (H1 could not safely retain a real traffic sample — see `docs/decisions.md`), not the docs alone. Same fail-closed discipline as the Anthropic codec on the **request** side (unrecognised item types block; image/file inputs block; malformed shapes block). On the **response** side, the contract the draft left unspecified is now stated (F16's recommendation): an **unrecognised event in a stream terminates the stream with an explicit error**, never passes through — a placeholder that reaches the user un-demasked is a worse failure than a truncated response. Retry/resumption tests per GROUND-14. | 3-5 | H1, H2a, H2b | Codec unit tests against H1's synthetic-structure fixtures (real fixture retention was not
achievable safely — see H1's own row); masked-request shape-preservation tests matching `mask_request.rs`'s own discipline; a replayed retry produces identical placeholder bindings and no duplicated output |
...
| **F7** | Which Codex auth mode does Veil support first? | (a) ChatGPT subscription; (b) API key; (c) both | Evidence-gated: H1 tests both. Note the modes are **not** interchangeable for interception — they target different hosts and base paths (D-H-3). If subscription auth fails through every interception mechanism, **(b)** is the honest launch scope | **ANSWERED WITHIN A USER-ACCEPTED, NAMED SCOPE, 2026-09-16 (H1 live-run):** **(a) ChatGPT subscription**, confirmed working and reproduced through two of three interception mechanisms tried (the third, plain `HTTPS_PROXY`, did not work — see `docs/decisions.md`). API-key mode was not tested this session; this narrowing was put to the human operator explicitly, together with H1's other scope deviations, and accepted — recorded as a real amendment to `veil-ecosystem`'s `INT-2026-09-14-001` ("UPDATE 2026-09-16"), not merely asserted in this repo's own docs. Not a clean, unconditional resolution of this fork's original two-mode text. See `docs/decisions.md`, "Track H, H1". |
...
| **F11** | **(NEW)** Codex redirect mode | (a) `openai_base_url` on the built-in provider; (b) custom `model_providers.veil`; (c) support both | **(a) first, (c) eventually.** OpenAI's own guidance prefers (a) for LLM proxies, and it preserves built-in provider identity; (b) may change capabilities and hide problems behind feature downgrades — which makes it a valuable *second* test case, not the first | **ANSWERED 2026-09-15/16 (H1 live-run):** **(a) `openai_base_url`** confirmed working and reproduced — **recommended as the primary mechanism on this evidence**, matching this row's own precedent. (b) custom `model_providers.veil` also confirmed working and reproduced — kept as a validated secondary path. Final production selection for `vg run codex` is deferred to H2a/H4, not decided by this recommendation alone. See `docs/decisions.md`, "Track H, H1". |
| **F12** | **(NEW)** SSE-only support vs WebSocket/capability parity | (a) SSE only, with `supports_websockets = false` pinned in the injected provider config and the downgrade disclosed; (b) full parity | **(a) for beta**, explicitly disclosed as a capability restriction Veil imposes — with H1 confirming whether pinning it actually prevents the WebSocket transport and whether `codex.transport.fallback_to_http` behaves. (b) is a real second transport implementation, unsized here | **PARTIALLY ANSWERED, 2026-09-15/16 (H1 live-run) — user-accepted as a named limitation:** `fallback_to_http` confirmed real and correct, but only for the failure mode H1 actually observed (a WS upgrade attempt getting a plain HTTP response back) — **not** for F12's own proposed mechanism (deliberately pinning `supports_websockets`/`prefer_websockets` false in an injected provider config). That specific mechanism was never tested; a real injection point exists (`prefer_websockets` confirmed present in the real model catalog) but exercising it is real, unstarted H3 work, not something this session settled. This gap was put to the human operator explicitly and accepted — recorded in `veil-ecosystem`'s `INT-2026-09-14-001` ("UPDATE 2026-09-16"). See `docs/decisions.md`, "Track H, H1". |
