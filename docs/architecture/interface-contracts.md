...
pub enum CredentialStatus { NotEnrolled, Installed(InstalledSigningCredential), EnvSeamShadowing }
pub enum EnrolError { EnvSeamActive, Profile(VaultError), AnchorMismatch, UnsupportedSignatureAlgorithm, NoPendingKey, AlreadyEnrolled { key_ref: KeyRef }, Keychain(VaultError) }
```

`request_device_signing_csr` generates a P-256 keypair in-process (never written to disk),
stores the private key in the OS keychain under a new content-addressed pending service
...
VaultError>` — splits "not yet enrolled" (`Ok(None)`, today's universal case) from genuine
misconfiguration (`Err`), which previously shared one `VaultError::Crypto(String)` shape
distinguishable only by message text. `sign_edge_event_record` and `SigningCredential`/
`DeviceSigningCredential` themselves are unchanged. New `vg-core` public type
`telemetry::signing::OwnedSigningCredential` (owned-storage counterpart to the borrowing
`SigningCredential<'a>`, for credentials held by long-lived structs like the emitter handle).
Downstream: `Engine::open` (`vg-adapters-claude`) now auto-detects a real device credential from
the OS keychain (gated on `VEIL_OBSERVATORY_ENDPOINT` being set) and threads it through to
production signing — the ECDSA arm is real and tested, but every real device still falls back to
HMAC today, since no enrolment flow exists yet. See `../decisions.md`'s 2026-08-31 "Config
surface" entry for the doubt-driven-development history (9 findings, all fixed).

**v1.6 addendum (2026-08-31, ADR-S):** `telemetry::signing::sign_edge_event_record`'s second
parameter changed from `key: &ReceiptSigningKey` to `credential: &SigningCredential<'_>` — a
new enum (`Hmac(&ReceiptSigningKey)` / `EcdsaP256(&DeviceSigningCredential)`) that now derives
`Integrity::algorithm` (and, for the ECDSA arm, `Integrity::key_ref`) from whichever credential
actually signs, rather than those being an independently-hard-coded literal and an
independently-caller-supplied `EdgeEventRecordInput::key_ref` field (removed) that could
previously disagree with the signing key. New public types: `SigningCredential`,
`DeviceSigningCredential` (a custodian-issued P-256 signing key plus its certificate-derived
`key_ref`/`device_ref`, constructed via `DeviceSigningCredential::from_parts(signing_key,
certificate_der, device_ref)` — `key_ref` is derived from `certificate_der` internally, not
accepted as a parameter, so it cannot be supplied independent of the key that signs). Downstream:
one call site, `vg-audit::telemetry_sink`'s `try_emit`, updated to wrap its existing
`ReceiptSigningKey` in `SigningCredential::Hmac(..)` — production still signs HMAC only; the
ECDSA arm is real and tested but not yet selected by any call site. See `../decisions.md`'s
2026-08-31 entry for the full doubt-driven-development history.
