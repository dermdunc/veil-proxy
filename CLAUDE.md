...
- Add a `docs/build-log/YYYY-MM-DD-<slug>.md` entry for any session with a real event worth
  telling (a decision, a failure, a fix, a surprising result) — see `docs/build-log/README.md`.
  This is distinct from `docs/session-log.md`: a readable, dated narrative for a reader without
  context, written deliberately, not generated by summarizing the technical log. Not every
  session needs one.
